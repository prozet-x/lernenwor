...
function RegistrationForm() {
    ...
    const checkUsernameExists = async (username) => {
        ...
        try {
            const response = await fetch(`/api/v1/users/checks/check-user-name-exists/${username}`);
            if (!response.ok) {
    ...
    const checkEmailExists = async (email) => {
        ...
        try {
            const response = await fetch(`/api/v1/users/checks/check-user-email-exists/${email}`);
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
            const data = await response.json();
